...
describe('Testing the book endpoints:', function () {
  ...
  it('It should not get a particular book with the invalid id', function (done) {
    var bookId = 888;
  ...
  it('It should not get a particular book with non-numeric id', function (done) {
    ...
    _chai["default"].request(_index["default"]).get("/api/v1/books/".concat(bookId)).set('Accpet', 'application/json').end(function (err, res) {
  ...
  it('It should not update a book with invalid id', function (done) {
    ...
    _chai["default"].request(_index["default"]).put("/api/v1/books/".concat(bookId)).set('Accpet', 'application/json').send(updatedBook).end(function (err, res) {
      expect(res.status).to.equal(404);
      res.body.should.have.property('message').eql("Cannot find book with the id: ".concat(bookId));
      done();
    });
  ...
  it('It should not delete a book with invalid id', function (done) {
    ...
    _chai["default"].request(_index["default"])["delete"]("/api/v1/books/".concat(bookId)).set('Accept', 'application/json').end(function (err, res) {
      expect(res.status).to.equal(404);
      res.body.should.have.property('message').eql("The book ".concat(bookId, " cannot be found"));
      done();
    });
  });
  it('It should not delete a book with non-numeric id', function (done) {
    ...
    _chai["default"].request(_index["default"])["delete"]("/api/v1/books/".concat(bookId)).set('Accpet', 'application/json').end(function (err, res) {
      expect(res.status).to.equal(400);
      res.body.should.have.property('message').eql('Please input a valid Book ID');
      done();
    });
